...
export default function QuestionBank() {
  ...
  const generateMutation = useMutation({
    mutationFn: () => api.post('/question-bank/generate', { moduleType: genType, difficulty: genDiff, count: genCount, category: genCategory || undefined }).then((r) => r.data),
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ['question-bank'] })
      setGenResult(data)
      setGenError('')
        ...
        <TabsContent value="generate">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">Automatically populate your bank with validated assessment questions by module type and difficulty.</p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {genResult && (
                <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm text-emerald-700">
                  <Sparkles className="h-4 w-4" /> {genResult.added} questions added to your bank.
                </div>
              )}
